...
public class RelationshipDelegate {
	...
	public List<RelationshipBean> getAll() {
		...
		for (models.Relationship person : modelPersonRels) {
			RelationshipBean relBean = PlayDozerMapper.getInstance().map(
					person, RelationshipBean.class);
	...
	public RelationshipBean getPersonRelationship(Long id) {
		...
		if (personRel != null) {
			RelationshipBean relBean = PlayDozerMapper.getInstance().map(
					personRel, RelationshipBean.class);
			return relBean;
		} else {
			return null;
		}
	...
	public void create(RelationshipBean relBean) {
		models.Relationship personRel = PlayDozerMapper.getInstance().map(
				relBean, models.Relationship.class);
	...
	public void update(RelationshipBean bean, Long id) {
		models.Relationship person = PlayDozerMapper.getInstance().map(bean,
				models.Relationship.class);
		person.update(id);
	...
	public List<RelationshipBean> getPersonRelationships(Long id) {
		List<models.Relationship> modelPersonRels = models.Relationship
				.findRelationshipsByPerson(id);
		List<RelationshipBean> pojosPersonRels = new ArrayList<RelationshipBean>();
	...
	public List<RelationshipBean> getPersonRelationshipsByType(Long id,
			String type) {

		List<models.Relationship> modelPersonRels = models.Relationship
				.findRelationshipsByPersonByType(id, type);
		List<RelationshipBean> pojosPersonRels = new ArrayList<RelationshipBean>();
		for (models.Relationship rel : modelPersonRels) {
			RelationshipBean relBean = PlayDozerMapper.getInstance().map(rel,
					RelationshipBean.class);
